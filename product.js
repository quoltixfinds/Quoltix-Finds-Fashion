// product.js
const products = [
  {
    id: 1,
    name: "Floral Midi Dress",
    image: "Media/Products/QF-FS-1.jpeg",
    category: "Dress",
    tag: "trending",
    sku: "QF-FS-1",
    gender: "female",
    affiliateLink: "https://affiliate.meesho.com/collection/MzA4NjQ1Mzo6Ojo6Om5vcm1hbA=="
  },
  {
    id: 2,
    name: "Striped Maxi Dress",
    image: "Media/Products/QF-FS-2.jpeg",
    category: "Dress",
    tag: "trending",
    sku: "QF-FS-2",
    gender: "female",
    affiliateLink: "https://affiliate.meesho.com/collection/MzA4NjQ4NDo6Ojo6Om5vcm1hbA=="
  },
  {
    id: 3,
    name: "Yellow Midi Dress",
    image: "Media/Products/QF-FS-3.jpeg",
    category: "Dress",
    tag: "trending",
    sku: "QF-FS-3",
    gender: "female",
    affiliateLink: "https://affiliate.meesho.com/collection/MzA4NjUxMTo6Ojo6Om5vcm1hbA=="
  },
  {
    id: 4,
    name: "Bodycon Midi Dress",
    image: "Media/Products/QF-FS-4.jpeg",
    category: "Dress",
    tag: "trending",
    sku: "QF-FS-4",
    gender: "female",
    affiliateLink: "https://affiliate.meesho.com/collection/MzMxMTY3MTo6Ojo6Om5vcm1hbA=="
  },
    {
    id: 5,
    name: "Striped Maxi Dress",
    image: "Media/Products/QF-FS-5.jpeg",
    category: "Dress",
    tag: "trending",
    sku: "QF-FS-5",
    gender: "female",
    affiliateLink: "https://affiliate.meesho.com/collection/MzM3NjAxNzo6Ojo6Om5vcm1hbA=="
  },

  // {
  //   id: 4,
  //   name: "Yellow Summer Dress",
  //   image: "https://via.placeholder.com/300x400?text=Yellow+Dress",
  //   category: "shirts",
  //   tag: "trending",
  //   sku: "QF104",
  //   gender: "female",
  //   affiliateLink: "https://example.com/affiliate4"
  // },
  // {
  //   id: 5,
  //   name: "Silver Earrings Set",
  //   image: "Media/Products/QF-FS-4.png",
  //   category: "Accessories",
  //   tag: "under999",
  //   sku: "QF-FS-4",
  //   gender: "unisex",
  //   affiliateLink: "https://example.com/affiliate5"
  // },
  // {
  //   id: 6,
  //   name: "Tote Bag with Strap",
  //   image: "Media/Products/QF-FS-5.png",
  //   category: "Bags",
  //   tag: "best",
  //   sku: "QF-FS-5",
  //   gender: "male",
  //   affiliateLink: "https://example.com/affiliate6"
  // },
  // {
  //   id: 7,
  //   name: "Black Striped Dress",
  //   image: "Media/Products/QF-FS-6.png",
  //   category: "Dress",
  //   tag: "best",
  //   sku: "QF-FS-6",
  //   gender: "female",
  //   affiliateLink: "https://example.com/affiliate7"
  // },
  // {
  //   id: 8,
  //   name: "Diamond Pendant",
  //   image: "Media/Products/QF-FS-7.png",
  //   category: "Jewelry",
  //   tag: "trending",
  //   sku: "QF-FS-7",
  //   gender: "unisex",
  //   affiliateLink: "https://example.com/affiliate8"
  // },
  // {
  //   id: 9,
  //   name: "Canvas Backpack",
  //   image: "Media/Products/QF-FS-8.png",
  //   category: "Bags",
  //   tag: "under999",
  //   sku: "QF-FS-8",
  //   gender: "male",
  //   affiliateLink: "https://example.com/affiliate9"
  // },
];
